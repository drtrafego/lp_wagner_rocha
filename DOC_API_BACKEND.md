...
Frontend (formulário)
        │
        │  POST /api/contact
        │  { name, email, whatsapp, utm_*, modelo?, ga_client_id?, ga_session_id? }
        ▼
┌──────────────────────────────────────────────────────────────────────┐
│  route.ts                                                            │
│                                                                      │
│  1. Valida payload com Zod                                           │
│  2. Captura IP · UserAgent · cookies _fbc/_fbp                       │
│     ga_client_id e ga_session_id vem do BODY (lidos pelo cliente)    │
...
└──────────────────────────────────────────────────────────────────────┘
        ...
        └──► GA4 Measurement Protocol  (google-analytics.com/mp/collect)
             Evento: generate_lead · client_id lido do cookie _ga no cliente
```
...
└── app/
    ├── layout.tsx           ← GTM instalado no <head> (unico responsavel pelo GA4)
    ├── page.tsx             ← Landing page principal
    ├── obrigado/
    │   └── page.tsx         ← Pagina de confirmacao com redirect automatico para WhatsApp
    └── api/
        └── contact/
            └── route.ts     ← POST /api/contact

drizzle.config.ts            ← Configuracao do drizzle-kit
playwright.config.ts         ← Configuracao dos testes E2E
tests/                       ← Testes Playwright (excluido do tsconfig)
```

> **IMPORTANTE:** Nao usar `GoogleAnalytics.tsx` (gtag.js direto). O GTM e o unico responsavel por carregar e gerenciar o GA4. Carregar gtag.js separado em paralelo com o GTM causa conflito: os eventos disparam direto para o GA4 sem passar pelo GTM, impedindo que aparecam no GTM Debug View e nas tags configuradas.
...
**Fallback de client_id:** se o cookie `_ga` nao existir, usa `server.{leadId}`. Eventos com client_id sintetico chegam ao GA4 mas nao aparecem em sessoes reais nem como conversoes nos relatorios.
...
Body (JSON):
{
  ...
  "ga_client_id":   string                          opcional — lido do cookie _ga no cliente
  "ga_session_id":  string                          opcional — lido do cookie _ga_XXXX no cliente
}
...
// ga_client_id e ga_session_id vem do body — mais confiaveis que cookie header com GTM
...
### GA4 — gerenciado exclusivamente pelo GTM

**NAO usar `GoogleAnalytics.tsx` (gtag.js separado).** O GTM e o unico responsavel por carregar o GA4. Carregar gtag.js em paralelo causa conflito: eventos disparam direto para o GA4 sem passar pelas tags do GTM, impedindo que apareçam no Debug View.

Eventos client-side sao enviados via `dataLayer.push`, que o GTM intercepta e encaminha ao GA4 pelas tags configuradas:
...
O `ga_client_id` e lido do cookie `_ga` no cliente e enviado no corpo da requisicao para o servidor usar no GA4 Measurement Protocol:
...
**Pagina /obrigado:** apos submit bem-sucedido o usuario e redirecionado para `/obrigado`, que exibe confirmacao por 3 segundos e redireciona automaticamente para o WhatsApp do cliente.

> O evento `generate_lead` e enviado client-side (via dataLayer/GTM) e server-side (Measurement Protocol).
...
| `obrigado/page.tsx` | URL do WhatsApp | Numero e mensagem pre-preenchida do cliente |

> **NAO adicionar `GoogleAnalytics.tsx` ao layout.** O GTM ja gerencia o GA4. Adicionar o componente cria conflito e impede eventos de aparecerem no GTM Debug View.
...
| `generate_lead` nao aparece no GTM Debug View | `GoogleAnalytics.tsx` carregando gtag.js separado do GTM | Remover o componente `GoogleAnalytics` — GTM deve ser o unico responsavel |
| Conversao nao aparece no GA4 mesmo com evento no dataLayer | GTM nao tem tag GA4 Evento configurada para `generate_lead` | Criar trigger "Evento personalizado: generate_lead" e tag GA4 Evento no GTM |
| GA4 MP envia evento mas nao aparece em sessoes/conversoes | `client_id` sintetico (`server.xxx`) sendo usado | Garantir que `ga_client_id` chegue no body — verificar se cookie `_ga` existe na pagina antes do submit |
